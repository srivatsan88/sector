...
def determine_cutoff_with_percentile(geometric_means, percentile=75):
    ...
    cutoff = np.percentile(geometric_means, percentile)
    return cutoff

def determine_cutoff_with_gmm(geometric_means, n_components=2):
    """Use Gaussian Mixture Models to determine the cutoff."""
...
def compute_summary_statistics(geometric_means):
    """Compute min, max, mean, median, and various percentiles."""
    statistics = {
...
# Main function to run threshold determination methods
def run_threshold_determination(geometric_means):

    # Step 2: Compute summary statistics
    stats = compute_summary_statistics(geometric_means)
    print("\n--- Summary Statistics ---")
    for key, value in stats.items():
        print(f"{key}: {value}")

    # Step 3: Compute thresholds using remaining methods
    print("\n--- Percentile Method (75th Percentile) ---")
    percentile_cutoff = determine_cutoff_with_percentile(geometric_means, percentile=75)
    print(f"Cutoff (75th Percentile): {percentile_cutoff}")

    print("\n--- Gaussian Mixture Model ---")
    gmm_cutoff, gmm_means = determine_cutoff_with_gmm(geometric_means)
    print(f"Cutoff (GMM): {gmm_cutoff}")
...
if __name__=="__main__":
    ...
    for docs in range(len(input_text_list)):
        ...
        for sentence in similar_sentences_json:
            ...
            comparison_result = json.loads(compare_sentences_flexible(input_sentence, reference_sentence, top_n=2))
    ...
    # Run the threshold determination methods
    threshold_results = run_threshold_determination(geometric_means)

    print("\n--- Final Threshold Results ---")
    for method, cutoff in threshold_results.items():
        if "cutoff" in method:
